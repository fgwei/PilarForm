...
class NewlineInferencer(delegate: WhitespaceAndCommentsGrouper) extends Iterator[Token] {
  ...
  private def followingTokenIsRecord(nextToken: Token): Boolean =
    nextToken != null && nextToken.tokenType == RECORD
...
object NewlineInferencer {
  ...
  private val TOKENS_WHICH_CAN_END_A_STATEMENT: Set[TokenType] = Set(
    INTEGER_LITERAL, FLOATING_POINT_LITERAL, CHARACTER_LITERAL, STRING_LITERAL, TRUE, FALSE, NULL, RETURN, RPAREN, RBRACKET, RBRACE)

  private val TOKENS_WHICH_CANNOT_BEGIN_A_STATEMENT: Set[TokenType] = Set(
    CATCH, EXTENDS, COMMA, DOT, SEMI, COLON, EQUALS,
    LBRACKET, RPAREN, RBRACKET, RBRACE)
